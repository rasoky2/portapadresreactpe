...
export default function ParentSettings() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const me = Array.isArray(list) ? list.find((u:any)=>u.IdUsuario===3) : null // TODO: obtener desde sesión real
        if (me) setData(me)
      } catch {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/users/${data.IdUsuario}`, { method: 'PUT', headers: { 'Content-Type':'application/json' }, body: JSON.stringify(payload) })
      if (!res.ok) {
