...
export default function CreateTeacherModal({ isOpen, onClose, onSave }: CreateTeacherModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showSuccess(
        'Docente creado', 
        `${formData.nombre} ${formData.apellido} ha sido registrado correctamente`
      )
      onClose()
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          <CardTitle className="text-xl font-semibold">Crear Nuevo Docente</CardTitle>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              <p className="text-sm text-blue-600">
                Una vez creado el docente, podrás asignarle materias y grados específicos desde la sección de asignaciones.
              </p>
            </div>
