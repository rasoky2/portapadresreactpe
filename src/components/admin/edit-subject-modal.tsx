...
interface EditSubjectModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (subjectId: number, subjectData: {
    nombreMateria: string
    idNivel: number
    horasSemanales: number
  }) => void
...
export default function EditSubjectModal({ isOpen, onClose, onSave, subject }: EditSubjectModalProps) {
  ...
  useEffect(() => {
    if (isOpen && subject) {
      setFormData({
        ...
        idNivel: subject.IdNivel,
        horasSemanales: subject.HorasSemanales
      })
      setErrors({})
    }
  }, [isOpen, subject])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await onSave(subject.IdMateria, formData)
      showSuccess(
        'Materia actualizada', 
        `${formData.nombreMateria} ha sido actualizada correctamente`
      )
      onClose()
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          <CardTitle className="text-xl font-semibold">Editar Materia</CardTitle>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              <p className="text-sm text-blue-600">
                Los cambios se aplicarán inmediatamente. Si cambias el nivel educativo, asegúrate de que las asignaciones existentes sean compatibles.
              </p>
            </div>
